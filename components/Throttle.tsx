...
export function Throttle({ onThrottleChange }: ThrottleProps) {
  ...
  const throttlePosition = useRef(new Animated.Value(throttleHeight - 40)).current;
  ...
  const panResponder = PanResponder.create({
    ...
    onPanResponderMove: (event, gestureState) => {
      trackRef.current?.measure((x, y, width, height, pageX, pageY) => {
        ...
        const newPercentage = Math.round(((throttleHeight - 40 - newPosition) / (throttleHeight - 40)) * 100);
  ...
  return (
    <View style={styles.container}>
      ...
      <View 
        ref={trackRef}
        style={[styles.track, { height: throttleHeight }]}>
        <Animated.View
